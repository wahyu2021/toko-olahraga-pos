<div>
    <x-slot name="header">
        <h1 class="text-xl font-semibold leading-tight text-gray-800">
            Laporan Keuangan - Cabang {{ Auth::user()->branch->name ?? 'N/A' }}
        </h1>
    </x-slot>
        ...
        <div class="max-w-full mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg p-4 sm:p-6">
                ...
                <div wire:loading.remove wire:target="triggerReportCalculation">
                    @if ($reportGenerated && Auth::user()->branch_id)
                        <div class="border-t pt-6">
                            ...
                            <div class="mt-8" wire:ignore x-data="{
                                ...
                                initChart() {
                                    ...
                                    const chartConfig = { type: 'line', data: this.chartData, options: { responsive: true, maintainAspectRatio: false, interaction: { mode: 'index', intersect: false }, scales: { y: { beginAtZero: true, ticks: { callback: (value) => 'Rp ' + new Intl.NumberFormat('id-ID').format(value) } } }, plugins: { tooltip: { callbacks: { label: (context) => (context.dataset.label || '') + ': Rp ' + new Intl.NumberFormat('id-ID').format(context.parsed.y) } } } } };
                                ...
                                <div class="bg-white rounded-lg p-4 sm:p-6 shadow-sm">
                                    ...
                                    <div class="h-96"><canvas x-ref="canvas"></canvas></div>
                                </div>
                            </div>
                                ...
                                <div class="overflow-x-auto">
                                    <table class="min-w-full divide-y divide-gray-200">
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200">
                                            ...
                                            @empty
                                                <tr>
                                                    <td colspan="5" class="text-center py-12 text-gray-500">Tidak ada
                                                        data transaksi pada rentang tanggal ini.</td>
                                                </tr>
                                            @endforelse
                                        </tbody>
                                    </table>
                                </div>
                                <div class="mt-4">{{ $salesDetails->links() }}</div>
                            </div>
                        </div>
                    @else
                        <div class="text-center py-12 text-gray-500">
                            <p>
                                @if (!Auth::user()->branch_id)
                                    Akun Anda tidak terhubung ke cabang manapun. Laporan tidak dapat ditampilkan.
                                @else
                                    Silakan klik "Tampilkan Laporan" untuk memuat data.
                                @endif
                            </p>
                        </div>
                    @endif
